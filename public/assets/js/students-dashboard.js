...
document.addEventListener('DOMContentLoaded', function () {
    fetch('/andaFP/public/assets/data/suggestions.json')
        ...
        .catch(error => {
            ...
            showError("No se pudieron cargar las sugerencias. Intente recargar la página.");
            document.getElementById('searchInput').placeholder = 'Error cargando datos...';
            document.getElementById('placeInput').placeholder = 'Error cargando datos...';
        });
});
...
function validateInputsValues(e, placeInput, searchInput) {
    ...
    if (!(validGrado && validPlace) && !(validGrado && placeValue === "") &&!(validPlace && searchValue ==="") && !(searchValue === "" && placeValue === "")) {
        showError("El contenido de los campos debe coincidir con alguna de las sugerencias.");
        e.preventDefault();
        return;
    }
