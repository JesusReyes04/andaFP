...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("/andaFP/public/assets/data/suggestions.json")
    ...
    .catch((error) => {
      ...
      document.getElementById("specialty").placeholder =
  ...
  form.addEventListener("submit", function (e) {
    ...
    const educationalCenter = document
      .getElementById("educational_center")
      .value.trim();
    ...
    if (password.length > 0) {
      if (!passwordRegex.test(password)) {
        errors.push(
          "La contraseña debe tener al menos 12 caracteres, incluyendo mayúsculas, minúsculas, un número y un carácter especial."
        );
      }
    ...
    if (
      profilePicture &&
      !["image/jpeg", "image/jpg", "image/png"].includes(profilePicture.type)
    ) {
...
function validateInputsValues(event, provinceInput, specialtyInput) {
  ...
  const validProvince = placeSuggestions.some(
    (p) => p.toLowerCase() === province
  );
